...
const SelectProfessional = () => {
  ...
  const { subServices, employee } = useSelector(
        ...
        <div className="flex flex-col gap-4">
          ...
          {matchedEmployee.map((emp, ind) => {
            ...
            return (
              <div
                key={ind}
                className={`flex items-center justify-between border-[2px] border-gray p-6 rounded-[18px] ${
                  isSelected ? "border-purple-800" : "border-gray"
                }`}
              >
                <div className="flex items-center  gap-3">
                  {/* <div>
                ...
                <div>
                  <button
                    ...
                    type="button"
                    className={`p-3 rounded-[16px] border-[1px] ${
                      isSelected ? "bg-purple-800 border-white" : "boder-gray"
                    }`}
                  >
                    {isSelected ? <CheckedIcon /> : <span>Select</span>}
                  </button>
                </div>
              </div>
            );
